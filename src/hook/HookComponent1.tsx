...
// - useRef : 특정한 상태를 기억하기 위한 함수, 값이 변경되더라도 렌더링을 수행하고 싶지 않을때,
//            DOM 요소를 기억하고 싶을 때 사용
// - useContext : 컴포넌트 트리 상에서 전역적으로 상태를 공유하고자 할때 사용하는 함수
// - useReducer : 상태 로직을 관리하는 함수, 상태 업데이트 로직을 컴포넌트 외부로 옮길 수 있게하여
//                코드 관리를 수월하게 도와줌 
// - useCallback : 메모리 내에서 함수를 재사용할 수 있도록 하는 함수,
//                 렌더링없이 특정 함수를 실행할 수 있게 함
...
export default function HookComponent1() {
    ...
    // - 개발 실행 환경에서는 mount - umount가 한번 실행되고난 후 다시 mount가 됨
    ...
    useEffect(() => {
        // 스코프할 상태 배열에 지정된 상태를 해당 effect에서 변경하면 무한 호출 됨
        // setState1(state1 + 1);
